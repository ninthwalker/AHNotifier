...
if ($settings.'Notification Types'.Telegram -eq "Enabled") {$telegram = $True} else {$telegram = $False}
if ($settings.'Notification Types'.Pushover -eq "Enabled") {$pushover = $True} else {$pushover = $False}
if ($settings.'Notification Types'.TextMsg -eq "Enabled") {$textMsg = $True} else {$textMsg = $False}
...
# Get Icon
$iconSearch = Invoke-Restmethod https://www.wowhead.com/item=$($details.Id) -UseBasicParsing
$end = '"'

If($iconSearch -match "https://wow.zamimg.com/images/wow/icons/.+(?=$end)") {
    $icon = $Matches[0]

    # icon filetype
    $iconFileType = $icon.Substring($icon.Length - 3)

    # get the icon
    try {
        Invoke-WebRequest -Uri $icon -OutFile "$env:TEMP\AHNotifier_Icon.$iconFileType"
        $iconImg = "$env:TEMP\AHNotifier_Icon.$iconFileType"
    }
    Catch {
        $iconImg = $imageFile
    }
...
# removed some words and updated part from the original msg
$toast_msg = $msg.TrimStart("AH Alert: ")
$toast_msg = $toast_msg -replace ".{29}$"

if (Test-Path Registry::HKEY_CLASSES_ROOT\AHNotifier\shell\open\command) {
    $openWOW = '<action activationType="protocol" arguments="AHNotifier:" content="Open WoW"/>'
}
else {
    $openWOW = ""
}

# Don't use this for now. Mosty people won't have the latest win ver for it.
# $timeStamp = $updatedAt | Get-Date -Format s

[xml]$ToastTemplate = @"
<toast activationType="protocol" launch="$($details.url)"  duration="long">
  <visual>
    <binding template="ToastGeneric">
      <text>Auction House Alert</text>
      <text>$toast_msg</text>
      <text>$updatedAt</text>
      <image placement="appLogoOverride" src="$iconImg"/>
      <text placement="attribution">AH Notifier</text>
    </binding>
  </visual>
  <actions>
    $openWOW
    <action activationType="protocol" arguments="$($details.url)" content="Details"/>
...
# note on API Limits:
# 50 API calls an hour
# 25 API Calls per 24 hours if using the BulkMode api endpoint (gets all realm AH data. Anywhere from like 2-10Mb depending on server)
# Each Item you check on the AH is 1 API call when NOT using the BulkMode
# If running this 24/7 then only run once per hour if using BulkMode. You can check however many items you want per hour though  this way.
# If running this 24/7 then only check for less than 50 items per hour when NOT using BulkMode.
